...
- All prior guards and signed-update tests continue to pass. Local operation was set to 20/25 with 50/day, no monthly cap, and no cooldown. The signed background updater delivered 0.2.5 → 0.2.6 and promoted it to the last known good release. The healthy worker automatically backfilled all seven existing compaction observations; all seven original attempt rows were unchanged, and SQLite integrity passed. No extra compaction was triggered for verification. Cache behavior under the new window will be evaluated from subsequent naturally eligible tasks.
